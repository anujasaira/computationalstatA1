...
For numerical responses, we average the predictors $\delta(x, L_k)$ to get an aggregated predictor $\delta_A(x)$.For class labels, we use voting among the predictors $\delta(x, L_k)$ to form the aggregated predictor $\delta_A(x)$.
...
![Decision Tree](imgclasstree.png){width="357"}
...
![Bagging Regession Tree](baggreg.png){width="322"}
...
![Figure 3. Misclassification Rate](largedataerr.png){width="244"}
...
Instead, when the predictors are highly variable the square of the mean might undere stimate the average value of the squared predictors because it doesn't account for the variability.
...
![Figure 1. Prediction error for subset selection and bagged subset selection](aleimm.png){width="481"}
...
*Bagging can improve only if the unbagged is not optimal*.Note that in all three graphs there is a point past which the bagged predictors have larger prediction error than the unbagged.The explenation is this: linear regression using all variables is a fairly stable procedure.The stability decreases as the number of variable used in the predictor decreases.*For a stable procedure* $\delta_B= \delta_A(\mathbf{x},P_L)$ *is not as accurate as* $\delta(\mathbf{x},P)$.As m decreases, the instability increases and there is a cross-over point at which $\delta^B_m$ becomes more accurate than $\delta_m$.
...
![Table diabetes dataset, number of bootstrap replicates](MissClassificationRate%20vs%20Num%20Bootstrap.png){width="307"} The diabates dataset was run using 10,25,50,100 replicates.
