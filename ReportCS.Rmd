...
$\(\ell\)$ A learning set $\mathcal{L}$ consists of data {($\mathit{y_n}$, $\mathit{x_n}$),$\mathit{n}$ = 1....
, $\mathit{N}$}
...
Let "\\4 = nr{k; \~(x, £#) = j} and take 9\~A(X) = argmaxjNj, that is, the j for which \~,\~-is maximum.
