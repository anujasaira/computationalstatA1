...
CLASSIFICATION TREE A classification tree is a structural mapping of binary decisions that lead to a decision about the class (interpretation) of an object (such as a pixel).
...
![Figure 1.Classification Tree](imgclasstree.png){width="500"} A classification tree is composed of branches that represent attributes, while the leaves represent decisions.
...
Last we create a Random Forest model using the same function but here we mtry as 3 ($\sqrt{8}$). So the best 3 variables among the 8 will be chosen.  
